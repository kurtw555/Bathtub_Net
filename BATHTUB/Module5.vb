...
Module Module5
	...
	Sub List_Fits()
		...
		For i As Double = 1 To 4
			If Iop(CInt(i)) > 0 Then
				ListFitOne(i)
			End If
		Next i

	End Sub
	Sub ListFitOne(ByVal io As Double)
		Dim fF, m As Double
		Dim nobs As Single
		Dim r2 As Double
		...
		With gLSht.Range("A3")
			...
			If Iop(CInt(io)) > 0 Then
				...
				For i As Double = 1 To m
					If Cobs(CInt(i), CInt(io)) > 0 And Cest(CInt(i), CInt(io)) > 0 And Izap(CInt(i)) = 0 Then
						x(1) = Math.Sqrt(MAx(0, CvCest(CInt(i), CInt(io)))) / Cest(CInt(i), CInt(io))
						x(2) = Math.Log(Cobs(CInt(i), CInt(io)) / Cest(CInt(i), CInt(io)))
	...
	Sub Oss(ByVal js As Double, ByVal iwT As Integer, ByRef osst As Double, ByRef r2 As Double, ByRef nobs As Single)
		Dim Weight As Double
		' weighted objective function for one variable, segment kc
		' residual mean square - adjusted for number of parameters
		'Dim r2 As Single
		Dim yo, yye As Single
		...
		For j As Double = 1 To Nseg
			If Cest(CInt(j), CInt(js)) > 0 And Cobs(CInt(j), CInt(js)) > 0 And Jzap(CInt(j)) <= 0 Then
				Weight = 1
				If (iwT = 2 Or iwT = 4) Then Weight *= Area(CInt(j))
				If (iwT >= 3) Then Weight = Ratv(Weight, MAx(CvCobs(CInt(j), CInt(js)), 0.001))
				Weight = Math.Sqrt(Weight)
				yo = Math.Log(Cobs(CInt(j), CInt(js))) * Weight
				yye = Math.Log(Cest(CInt(j), CInt(js))) * Weight
				osst += (yye - yo) ^ 2
				f0 += 1
				f1 += yo
				f2 += yo ^ 2
	...
	Sub GlobalCalib(ByVal jo As Integer)
		...
		Select Case jo
			...
			Case 1
				...
				For k As Double = 1 To 5
					...
					If k = 1 And Nseg > 1 Then
						nU += 1
						iuK(nU) = 13
					ElseIf Iop(CInt(k)) > 0 And k <> 5 Then 
						nU += 1
						iuK(nU) = k - 1
					End If
				Next k
				If nU > 0 Then LeastSquaresGlobal()
			...
			Case 2
				...
				txt = ""
				For k As Double = 1 To 4
					If frmCalibration.DefInstance.chkVariable(CInt(k - 1)).CheckState <> CheckState.Unchecked Then
						nU = 1
			...
			Case 3
				...
				For i As Double = 1 To 5
					If Iwork(CInt(i)) > 0 Then
						nuC += 1
						iuC(nuC) = i
					End If
				Next i
				If nuC = 0 Then Exit Sub

				' SelectCalibVariables
				nU = 0
				For i As Double = 1 To 4
					If Icoef(CInt(i)) > 0 Then
						nU += 1
						iuK(nU) = i - 1
	...
	Sub CalibHeader()
		'header for calibration progress window
		txt = txt & "Calibrated Variable:" & DiagName(iuC(1)) & Environment.NewLine
		'For i = 1 To nU
		...
		'     Next i
		txt = txt & "Iter    Resid SS Coefficients..." & Environment.NewLine
	End Sub

	Sub Dfunc(ByVal pp() As Double, ByVal dd() As Double, ByRef f0 As Double)
		Dim fF, plast As Double
		...
		For k As Integer = 1 To nU
			txt = txt & FormatF(Math.Exp(pp(k)), " ####.00")
		Next k
		txt = txt & Environment.NewLine

		With frmCalibration.DefInstance.txtCalib
			.Text = txt
			.SelectionStart = 0
		End With
	...
	Function Func(ByVal pp() As Double) As Double
		Dim result As Double = 0
		Dim fF As Double
		Dim nobs As Single
		Dim ppu, r2 As Double
		...
		For i As Integer = 1 To nU
			...
			Else
				...
				Else
					'c by segment group
					For j As Double = 1 To Nseg
						If Jsg(i) = Iag(CInt(j)) And Izap(CInt(j)) = 0 Then Cal(CInt(j), kC) = ppu
					Next j
				End If
			End If
		Next i
		...
		For i As Integer = 1 To nuC
			Oss(iuC(i), iwT, fF, r2, nobs)
			result += fF
		Next i
	...
	Function IndSum(ByVal iv() As Integer, ByVal m As Integer) As Integer
		'sum index
		Dim result As Integer = 0
		For i As Integer = 1 To m
			If iv(i) > 0 Then result += 1
		Next i
		Return result
	End Function

	Sub LocalCalib(ByVal io As Integer)
		...
		For j As Integer = 1 To 4
			If frmCalibration.DefInstance.chkVariable(j - 1).CheckState <> CheckState.Unchecked Then
	...
	Sub LeastSquaresLocal(ByVal jc As Integer, ByVal kr As Integer)
		...
		For i As Double = 1 To Nseg
			...
			Jzap(CInt(i)) = 1
			If ListBoxHelper.GetSelected(frmCalibration.DefInstance.List1, i - 1) And Cobs(CInt(i), jc) > 0 And Cal(CInt(i), kC) > 0 Then
				Jzap(CInt(i)) = 0

				If Iout(CInt(i)) > 0 Or jc <> 1 Then
					'c by segment group
					If jR > 0 Then
						For j As Integer = 1 To nU
							If Jsg(j) = Iag(CInt(i)) Then
								Cal(CInt(i), kC) = Math.Exp(Xp(j))
								GoTo s35
							End If
						Next j
						nU += 1
...
s35:
	...
	Sub ListCalibFactors()
		...
		line_no = 0
		ExcelApp.Sheets("CalibFactors").Activate()
		With ExcelApp.Range("A1")
			.Range("A1:z5000").Clear()
			ExcelApp.Range("header_listcal").Copy()
			...
			ExcelApp.ActiveSheet.Paste(.Offset(0, 0))
	...
	Sub ExCo()
		Dim delta, imin, j, XMIN As Double
		...
		If Iop(1) <= 0 Then
			'UPGRADE_WARNING: (2081) Return has a new behavior. More Information: https://docs.mobilize.net/vbuc/ewis/warnings#id-2081
			Return 
		End If
		Icalc = 0
		line_no = 0

		ExcelApp.Sheets("work").Activate()
		With ExcelApp.Sheets("work").Range("A1")
...
S56:
			For i As Double = 1 To Nseg
				...
				If Iout(CInt(i)) = 0 Or Cal(CInt(i), 1) = 0 Then
					isym(CInt(i)) = "*"
				Else
					isym(CInt(i)) = " "
					Cest(CInt(i), 1) = Cobs(CInt(i), 1)
				End If
				'c target
				Yp(CInt(i)) = Cest(CInt(i), 1)

				'c mass-balance terms
				Balas(i, 1, 2) '??????

				'c left-hand side = external inputs + net advective inputs
				Xp(CInt(i)) = Bt(1) - Bt(11) + Bt(2) + Bt(6) - Bt(7) - Bt(4)
				...
				j = Iout(CInt(i))
				If j > 0 Then
					delta = Cest(CInt(i), 1) - Cest(CInt(j), 1)
					A(CInt(i), CInt(i)) = delta
					A(CInt(j), CInt(i)) = -delta
				End If
			Next i

			'c eliminate segments with null diagonal term
			For i As Double = 1 To Nseg
				If A(CInt(i), CInt(i)) = 0 Or Cal(CInt(i), 1) = 0 Then
					For j = 1 To Nseg
						A(CInt(j), CInt(i)) = 0
						A(CInt(i), CInt(j)) = 0
					Next j
			...
			For i As Double = 1 To Nseg
				A(CInt(i), Nseg + 1) = Xp(CInt(i))
			Next i
			SolveLinear()
			For i As Double = 1 To Nseg
				Xp(CInt(i)) = A(CInt(i), Nseg + 1)
			Next i
			...
			For i As Double = 1 To Nseg
				...
				Xp(CInt(i + Nseg)) = Exch(CInt(i))
				If i = imin Then
					Cal(CInt(i), 1) = 0
					'c if solution >0, recalibrate
				ElseIf Xp(CInt(i)) > 0 Then 
					Cal(CInt(i), 1) *= Ratv(Xp(CInt(i)), Exch(CInt(i)))
				End If
			Next i
			...
			ExcelApp.Range("header_exchange").Copy()
			...
			ExcelApp.ActiveSheet.Paste(ExcelApp.ActiveSheet.Range("A1").Offset(line_no, 0))
			'c Results
			'       write(nout,1)
			For i As Double = 1 To Nseg
	...
	Function Runi1() As Object
		Return ExcelApp.Application.WorksheetFunction.Rand()
	End Function

	Sub MinCon()
		Dim fF, freT As Double
		Dim itj, km As Integer
		...
		For k As Integer = 1 To 11
			For i As Integer = 1 To nU
				Xp(i) = Yp(i) + k - 6
			Next i
			fF = Func(ArraysHelper.CastArray(Of Double())(Xp))
		...
		For iT = 1 To nbuZ
			...
			nD = 0
			'c solution
			DFPMI2(ArraysHelper.CastArray(Of Double())(Xp), nU, ptoL, itj, freT)
		...
		nD = 0
		deL = deL2
		DFPMI2(ArraysHelper.CastArray(Of Double())(Yp), nU, ptoL2, itj, freT)
	...
	Sub DFPMI2(ByVal p2() As Double, ByVal n2 As Integer, ByVal ftoL As Double, ByRef iteR2 As Integer, ByRef freT As Double)
		Dim dp, faC, faD, faE, fP As Double
		'c modified by www
		...
		'     *HDG(NMAX)
		Dim itmax As Integer = 200
		Dim dpmax As Double = 0.3
		Dim psave(50) As Single
		Dim hessin(50, 50) As Single
		Dim xi(50) As Single, g(50) As Single, dg(50) As Single, hdg(50) As Single

		'c not needed since dfunc returns function
		'c FP = Func(P)
		Dfunc(p2, ArraysHelper.CastArray(Of Double())(g), fP)
		...
		For itS As Integer = 1 To itmax
			iteR2 = itS
			LinMin(p2, ArraysHelper.CastArray(Of Double())(xi), n2, freT)
			...
			For i As Integer = 1 To n2
				If (Math.Abs(p2(i) - psave(i)) > ftoL) Then GoTo S122
			Next i
			Exit Sub
S122:
			fP = freT
			...
			'c freT = Func(P)
			Dfunc(p2, ArraysHelper.CastArray(Of Double())(g), freT)
			For i As Integer = 1 To n2
				dg(i) = g(i) - dg(i)
			Next i
	...
	Sub LinMin(ByVal p2() As Double, ByVal xi() As Double, ByVal n2 As Integer, ByRef freT As Double)
		Dim F1dimX As Object
		Dim fA, fB, fX, XMIN As Double
		'      PARAMETER (NMAX=50,TOL=1.E-4)
		'      EXTERNAL F1DIM
		'      DIMENSION P(N), XI(N)
		'      COMMON /F1COM/ NCOM,PCOM(NMAX),XICOM(NMAX)
		Dim Tol2 As Double = 0.0001
		...
		Dim xx As Double = 1
		Dim bX As Double = 2
		MNBRAK(aX, xx, bX, fA, fX, fB, F1dimX)
	...
	Function Sign2(ByVal aa As Double, ByVal bb As Double) As Double
	...
	Sub MNBRAK(ByRef aX As Double, ByRef bX As Double, ByRef cX As Double, ByRef fA As Double, ByRef fB As Double, ByRef fC As Double, ByVal funcXZ As Object)
		Dim dDum, fU, q2, r, u, uliM As Double
		'      PARAMETER (GOLD=1.618034, TINY=1.E-20)
		'c was glimit=100
		'      Parameter (GLIMIT = 10#)
		Dim glimit As Integer = 10
		Dim gold As Double = 1.618034
		Dim tiny As Double = 1E-20
...
S1:
		If fB >= fC Then
			r = (bX - aX) * (fB - fC)
			q2 = (bX - cX) * (fB - fA)
			u = bX - ((bX - cX) * q2 - (bX - aX) * r) / (2 * Sign2(MAx(Math.Abs(q2 - r), tiny), q2 - r))
	...
	Function Brent(ByVal aX As Double, ByVal bX As Double, ByVal cX As Double, ByVal F As Object, ByVal Tol3 As Double, ByRef XMIN As Double) As Double
		Dim d, Etemp, fU, p2, q2, r, Tol1, Tol2, u, xM As Double
		'      PARAMETER (ITMAX=100,CGOLD=.3819660,ZEPS=1.0E-10)
		Dim itmax As Integer = 100
		Dim cgold As Double = 0.381966
		Dim zeps As Double = 0.0000000001

		Dim a2 As Single = MIn(aX, cX)
		Dim b As Double = MAx(aX, cX)
		Dim v As Double = bX
		Dim w As Double = v
		Dim x1 As Double = v
		Dim e2 As Double = 0
		Dim fX As Double = F1dim(x1)
		Dim Fv2 As Double = fX
		Dim Fw As Double = fX
		For iteR2 As Integer = 1 To itmax
			xM = 0.5 * (a2 + b)
			Tol1 = Tol3 * Math.Abs(x1) + zeps
			Tol2 = 2# * Tol1
			If (Math.Abs(x1 - xM) <= (Tol2 - 0.5 * (b - a2))) Then GoTo S3
			If Math.Abs(e2) > Tol1 Then
				r = (x1 - w) * (fX - Fv2)
				q2 = (x1 - v) * (fX - Fw)
				p2 = (x1 - v) * q2 - (x1 - w) * r
				q2 = 2 * (q2 - r)
				...
				Etemp = e2
				e2 = d
				If (Math.Abs(p2) >= Math.Abs(0.5 * q2 * Etemp) Or p2 <= q2 * (a2 - x1) Or p2 >= q2 * (b - x1)) Then GoTo S1
				d = p2 / q2
				u = x1 + d
...
S3:
	...
	End Function
	Function F1dim(ByVal x2 As Double) As Double
		'      Parameter (NMAX = 50)
		'      COMMON /F1COM/ NCOM,PCOM(NMAX),XICOM(NMAX)


		For j As Integer = 1 To ncoM
			xT(j) = pcoM(j) + x2 * xicoM(j)
		Next j
		Return Func(ArraysHelper.CastArray(Of Double())(xT))
	End Function
End Module