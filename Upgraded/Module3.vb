Option Strict Off
Option Explicit On
Imports Excel = Microsoft.Office.Interop.Excel
Imports System
Imports System.Text
Imports UpgradeHelpers.Helpers
Module Module3
	Sub Output_Init()
		'initialize output list parameters

		ShowWarnings = True

		NGlobals = 4
		GlobalName(1) = "AVERAGING PERIOD (YRS)"
		GlobalName(2) = "PRECIPITATION (METERS)"
		GlobalName(3) = "EVAPORATION (METERS)"
		GlobalName(4) = "INCREASE IN STORAGE (METERS)"

		'input concentrations
		NVariables = 5
		VariableName(1) = "CONSERVATIVE SUBST."
		VariableName(2) = "TOTAL P"
		VariableName(3) = "TOTAL N"
		VariableName(4) = "ORTHO P"
		VariableName(5) = "INORGANIC N"

		'map components
		Imap(1) = 1
		Imap(2) = 6
		Imap(3) = 7

		'mass balance type labels
		MassBalName(0) = "Observed"
		MassBalName(1) = "Predicted"

		'type codes
		N_Type_Codes = 5
		Type_Code(1) = "Monitored Inflow"
		Type_Code(2) = "Non Point Inflow"
		Type_Code(3) = "Point Source"
		Type_Code(4) = "Reservoir Outflow"
		Type_Code(5) = "Downstream Boundary"
		'Type_Code(6) = "Internal Load"


		'term names
		NtermS = 18
		TermName(1) = "PRECIPITATION"
		TermName(2) = "EXTERNAL INFLOW"
		TermName(3) = "***EVAPORATION"
		TermName(4) = "GAUGED OUTFLOW"
		TermName(5) = "***RETENTION"
		TermName(6) = "ADVECTIVE INFLOW"
		TermName(7) = "ADVECTIVE OUTFLOW"
		TermName(8) = "DIFFUSIVE INFLOW"
		TermName(9) = "***TOTAL INFLOW"
		TermName(10) = "***TOTAL OUTFLOW"
		TermName(11) = "***STORAGE INCREASE"
		TermName(12) = "DIFFUSIVE OUTFLOW"
		TermName(13) = "NET DIFFUSIVE INFLOW"
		TermName(14) = "NET DIFFUSIVE OUTFLOW"
		TermName(15) = "INTERNAL LOAD"
		TermName(16) = "TRIBUTARY INFLOW"
		TermName(17) = "NONPOINT INFLOW"
		TermName(18) = "POINT-SOURCE INFLOW"

		'C SEGMENT MASS BALANCE TERMS TO BE PRINTED FORMAT 1

		Njord = 15
		Jord(1) = 1
		Jord(2) = 15
		Jord(3) = 16
		Jord(4) = 17
		Jord(5) = 18
		Jord(6) = 6
		Jord(7) = 13
		Jord(8) = 9
		Jord(9) = 4
		Jord(10) = 7
		Jord(11) = 14
		Jord(12) = 10
		Jord(13) = 3
		Jord(14) = 11
		Jord(15) = 5
		'      data njord/15/
		'      data jord/1,15,16,17,18,6,13,9,4,7,14,10,3,11,5/

		'C GROSS MASS BALANCE TERMS TO BE PRINTED

		Nkord = 14
		Kord(1) = 1
		Kord(2) = 15
		Kord(3) = 16
		Kord(4) = 17
		Kord(5) = 18
		Kord(6) = 13
		Kord(7) = 9
		Kord(8) = 4
		Kord(9) = 7
		Kord(10) = 14
		Kord(11) = 10
		Kord(12) = 3
		Kord(13) = 11
		Kord(14) = 5

		'   DATA NKORD/14/,KORD/1,15,16,17,18,13,9,4,7,14,10,3,11,5,1*0/

		'mass balance terms format2
		'  DATA MORD/8/,LORD/2,1,6,11,7,4,8,3,0,0/

		Mord = 8
		Lord(1) = 2
		Lord(2) = 1
		Lord(3) = 6
		Lord(4) = 11
		Lord(5) = 7
		Lord(6) = 4
		Lord(7) = 8
		Lord(8) = 3

	End Sub

	Sub List_All()
		Dim sN As String = ""
		'MsgBox( "minimizing Excel Template"
		Dim realversion As Double = CDbl(Wka.Version)
		If (realversion < 15) Then Wka.WindowState = Excel.XlWindowState.xlMinimized
		For i As Integer = 0 To 9
			Select Case i
				Case 9 'reversed the order to accomodate excel .ADD behavior
					List_Inputs()
				Case 8
					List_Tree()
				Case 7
					List_Hydraulics()
				Case 6
					List_GrossBalances()
				Case 5
					If Not NoviceUser Then List_SegBalances()
				Case 4
					If Not NoviceUser Then List_Terms()
				Case 3
					List_Diagnostics(1)
				Case 2
					If Not NoviceUser Then List_Profiles()
				Case 1
					If Not NoviceUser Then List_TTests()
				Case 0
					If Not NoviceUser Then List_Fits()

			End Select

			sN = gLSht.Name
			Status("Listing: " & sN)
			'MsgBox( ("About to Copy " & sN)
			SheetCopy((sN))
		Next i
		Status("Ready")
		Wka.WindowState = Excel.XlWindowState.xlNormal

	End Sub 'List_All

	Sub StartSheet(ByVal sN As String) 'sN is Sheet Name
		'start new output sheet
		'    If DebugMode Then MsgBox( ("Loading Excel from Module3, StartSheet " & sN)
		'UPGRADE_ERROR: (1010) The preceding line couldn't be parsed. More Information: https://docs.mobilize.net/vbuc/ewis#id-#1010
		'LoadExcel
		'MsgBox( "N22: StartSheet (module3) is Calling ClearOutput"
		'ClearOutput
		Status((sN))
		Wkb.Sheets(sN).Cells.Clear()
		'MsgBox( "Cleared Cells in " & Wkb.Name & " " & sN
		gLSht = Wkb.Sheets(sN)
		With gLSht
			'     If DebugMode Then MsgBox( "StartSheet Clearing: " & Wkb.Name & "." & gLSht.Name & "  " & Title
			'UPGRADE_ERROR: (1010) The preceding line couldn't be parsed. More Information: https://docs.mobilize.net/vbuc/ewis#id-#1010
			'.Cells.Clear
			gLSht.Range("A1").Value = Title
			gLSht.Range("A1", "B2").Font.Bold = True
			gLSht.Range("A2").Value = "File:"
			gLSht.Range("B2").Value = CaseFile
		End With
	End Sub
	Sub List_Hydraulics()

		Dim atoT, vtoT As Single

		StartSheet("Hydraulics")
		line_no = 0
		With gLSht.Range("A4")
			' Copy the header for hydraulics
			Hdr.Range("Header_Hydrau").Copy()
			gLSht.Paste(.Offset(line_no, 0))

			line_no += 3
			For i As Double = 1 To Nseg
				line_no += 1
				Hydrau(i)
				.Offset(line_no, 0).Value = i
				.Offset(line_no, 0).HorizontalAlignment = Excel.Constants.xlCenter
				.Offset(line_no, 1).Value = SegName(CInt(i))
				.Offset(line_no, 2).Value = Iout(CInt(i))
				.Offset(line_no, 2).HorizontalAlignment = Excel.Constants.xlCenter
				.Offset(line_no, 3).Value = Qnet(CInt(i))
				.Offset(line_no, 3).NumberFormat = "0.0"
				For k As Integer = 3 To 8
					.Offset(line_no, 1 + k).Value = x(k)
					If k = 3 Then
						.Offset(line_no, 1 + k).NumberFormat = "0.0000"
					Else
						.Offset(line_no, 1 + k).NumberFormat = "0.0"
					End If
				Next k
			Next i

			line_no += 2
			atoT = 0
			vtoT = 0
			CType(Hdr, Excel.Worksheet).Range("Header_Morpho").Copy()
			CType(gLSht, Excel.Worksheet).Paste(.Offset(line_no, 0))
			line_no += 2
			For i As Double = 1 To Nseg
				line_no += 1
				.Offset(line_no, 0).Value = i
				.Offset(line_no, 0).HorizontalAlignment = Excel.Constants.xlCenter
				.Offset(line_no, 1).Value = SegName(CInt(i))
				x(1) = Area(CInt(i))
				x(2) = Zmn(CInt(i))
				x(3) = Zmx(CInt(i))
				x(4) = Slen(CInt(i))
				x(5) = Area(CInt(i)) * Zmn(CInt(i))
				atoT += Area(CInt(i))
				vtoT += x(5)
				x(6) = Ratv(Area(CInt(i)), Slen(CInt(i)))
				x(7) = Ratv(Slen(CInt(i)), x(6))
				For j As Integer = 1 To 7
					.Offset(line_no, j + 1).Value = x(j)
					.Offset(line_no, j + 1).NumberFormat = "0.0"
				Next j
			Next i
			line_no += 1
			.Offset(line_no, 0).Value = "Totals"
			.Offset(line_no, 2).NumberFormat = "0.0"
			.Offset(line_no, 3).NumberFormat = "0.0"
			.Offset(line_no, 6).NumberFormat = "0.0"
			.Offset(line_no, 2).Value = atoT
			.Offset(line_no, 6).Value = vtoT
			.Offset(line_no, 3).Value = Ratv(vtoT, atoT)
		End With
	End Sub

	Sub List_GrossBalances()
		Dim Export, i, jseG, k As Double
		Dim Tf As Single

		Dim Da(20) As Single
		Dim runofF, vaR, xL, cV, cU, pV1, pL, cvQ, qU, tW As Single

		'C kPRINT=3 GROSS WATER AND NUTRIENT BALANCES

		'c water balances
		If NTrib <= 0 Then Exit Sub
		'Call Balan
		MassBalanceTerms()

		StartSheet("Overall Balances")
		line_no = 0
		With gLSht.Range("A4")
			' Copy the header for gross water balance
			Hdr.Range("Header_gwbal").Copy()
			gLSht.Paste(.Offset(line_no, 0))

			.Offset(line_no + 2, 7).Value = P(1)
			line_no += 4

			For i = 1 To 18
				Da(CInt(k)) = 0
			Next i

			For i = 1 To NTrib
				If Icode(CInt(i)) <= 4 Then
					If Icode(CInt(i)) = 4 Then
						k = 4
					Else
						k = 15 + Icode(CInt(i))
					End If
					Da(CInt(k)) += Darea(CInt(i))
					qU = Flow(CInt(i))
					cvQ = CvFlow(CInt(i))
					If Icode(CInt(i)) = 2 Then
						qU = FlowL(CInt(i))
						cvQ = CvFlowL(CInt(i))
					End If
					runofF = Ratv(qU, Darea(CInt(i)))
					vaR = (cvQ * qU) ^ 2
					If qU > 0 Then
						line_no += 1
						.Offset(line_no, 0).Value = i
						.Offset(line_no, 0).HorizontalAlignment = Excel.Constants.xlCenter
						.Offset(line_no, 1).Value = Icode(CInt(i))
						.Offset(line_no, 1).HorizontalAlignment = Excel.Constants.xlCenter
						.Offset(line_no, 2).Value = Iseg(CInt(i))
						.Offset(line_no, 2).HorizontalAlignment = Excel.Constants.xlCenter
						.Offset(line_no, 3).Value = TribName(CInt(i))
						If Darea(CInt(i)) > 0 Then .Offset(line_no, 4).Value = Darea(CInt(i))
						.Offset(line_no, 4).NumberFormat = "0.0"
						.Offset(line_no, 5).Value = qU
						.Offset(line_no, 5).NumberFormat = "0.0"
						.Offset(line_no, 6).Value = vaR
						.Offset(line_no, 6).NumberFormat = "0.00E+00"
						If qU > 0 Then .Offset(line_no, 7).Value = cvQ
						.Offset(line_no, 7).NumberFormat = "0.00"
						If runofF > 0 Then .Offset(line_no, 8).Value = runofF
						.Offset(line_no, 8).NumberFormat = "0.00"
					End If
				End If
			Next i

			'C DRAINAGE AREAS
			'c total external
			Da(2) = Da(16) + Da(17) + Da(18)
			'c precip
			Da(1) = Area(Nseg + 1)
			'C TOTAL IN
			Da(9) = Da(1) + Da(2)
			'C put da error in advective outflow
			Da(7) = Da(9) - Da(4)
			Da(10) = Da(9)
			Da(5) = 0


			'c Summary
			For j As Integer = 1 To Nkord
				i = Kord(j)
				If Not (i = 5 Or i = 15 Or Term(1, CInt(i)) = 0) Then
					line_no += 1
					runofF = Ratv(Term(1, CInt(i)), Da(CInt(i)))
					cV = MIn(Ratv(Math.Sqrt(CvTerm(1, CInt(i))), Math.Abs(Term(1, CInt(i)))), 9.99)
					With gLSht.Range("A4")
						.Offset(line_no, 0).Value = TermName(CInt(i))
						If Da(CInt(i)) > 0 Then .Offset(line_no, 4).Value = Da(CInt(i))
						.Offset(line_no, 4).NumberFormat = "0.0"
						.Offset(line_no, 5).Value = Term(1, CInt(i))
						.Offset(line_no, 5).NumberFormat = "0.0"
						.Offset(line_no, 6).Value = CvTerm(1, CInt(i))
						.Offset(line_no, 6).NumberFormat = "0.00E+00"
						.Offset(line_no, 7).Value = cV
						.Offset(line_no, 7).NumberFormat = "0.00"
						If runofF > 0 Then .Offset(line_no, 8).Value = runofF
						.Offset(line_no, 8).NumberFormat = "0.00"
					End With
				End If

			Next j

			'c gross nutrient balances

			For iC As Integer = 1 To 3
				If Iop(iC) > 0 Then

					line_no += 3
					Hdr.Range("Header_Gross").Copy()
					gLSht.Paste(gLSht.Range("A4").Offset(line_no, 0))

					gLSht.Range("A4").Offset(line_no, 4).Value = MassBalName(Iop(11))
					gLSht.Range("A4").Offset(line_no + 1, 4).Value = VariableName(Imap(iC))
					line_no += 3
					tW = Term(iC + 1, 9) 'total load
					Tf = Term(1, 9) 'total flow

					For i = 1 To NTrib
						If Icode(CInt(i)) < 5 And Iseg(CInt(i)) > 0 Then
							'    If (Icode(i) >= 5 Or Iseg(i) <= 0) Then Next i
							jseG = Iseg(CInt(i))
							If Icode(CInt(i)) = 2 Then
								cU = Concil(CInt(i), Imap(iC))
								cV = CvCil(CInt(i), Imap(iC))
								qU = FlowL(CInt(i))
								cvQ = CvFlowL(CInt(i))
							Else
								cU = Conci(CInt(i), Imap(iC))
								cV = CvCi(CInt(i), Imap(iC))
								qU = Flow(CInt(i))
								cvQ = CvFlow(CInt(i))
							End If
							'If Icode(i) = 4 And Iop(11) = 1 Then
							'    cU = Cest(jseG, iC)
							If Icode(CInt(i)) = 4 Then
								cU = Cuse(Cobs(CInt(jseG), iC), Cest(CInt(jseG), iC), Iop(11))
								cV = Ratv(Math.Sqrt(CvCest(CInt(jseG), iC)), cU)
							End If
							xL = qU * cU
							vaR = (cV ^ 2 + cvQ ^ 2) * xL ^ 2
							Export = Ratv(xL, Darea(CInt(i)))
							cV = Ratv(Math.Sqrt(vaR), xL)
							pL = Ratv(xL, tW)
							pV1 = Ratv(vaR, CvTerm(iC + 1, 9))
							If xL > 0 Then
								line_no += 1
								With gLSht.Range("A4")
									.Offset(line_no, 0).Value = i
									.Offset(line_no, 0).HorizontalAlignment = Excel.Constants.xlCenter
									.Offset(line_no, 1).Value = Icode(CInt(i))
									.Offset(line_no, 1).HorizontalAlignment = Excel.Constants.xlCenter
									.Offset(line_no, 2).Value = Iseg(CInt(i))
									.Offset(line_no, 2).HorizontalAlignment = Excel.Constants.xlCenter
									.Offset(line_no, 3).Value = TribName(CInt(i))
									.Offset(line_no, 4).Value = xL
									.Offset(line_no, 4).NumberFormat = "0.0"
									If pL > 0 And Icode(CInt(i)) <> 4 Then
										.Offset(line_no, 5).Value = pL
										.Offset(line_no, 5).NumberFormat = "0.0%"
									End If
									.Offset(line_no, 6).Value = vaR
									.Offset(line_no, 6).NumberFormat = "0.00E+00"
									If pV1 > 0 And Icode(CInt(i)) <> 4 Then
										.Offset(line_no, 7).Value = pV1
										.Offset(line_no, 7).NumberFormat = "0.0%"
									End If
									.Offset(line_no, 8).Value = cV
									.Offset(line_no, 8).NumberFormat = "0.00"
									If cU > 0 Then
										.Offset(line_no, 9).Value = cU
										.Offset(line_no, 9).NumberFormat = "0.0"
									End If
									If Export > 0 Then
										.Offset(line_no, 10).Value = Export
										.Offset(line_no, 10).NumberFormat = "0.0"
									End If
								End With
							End If
						End If
					Next i
					'summary terms
					For j As Integer = 1 To Nkord
						i = Kord(j)
						If i <> 3 And Term(iC + 1, CInt(i)) <> 0 Then
							cU = 0
							Export = 0
							cV = 0
							If Not (i = 5 Or i = 15) Then
								cU = Ratv(Term(iC + 1, CInt(i)), Term(1, CInt(i)))
								Export = Ratv(Term(iC + 1, CInt(i)), Da(CInt(i)))
							End If
							cV = Ratv(Math.Sqrt(CvTerm(iC + 1, CInt(i))), Math.Abs(Term(iC + 1, CInt(i))))
							cV = MIn(cV, 9.999)
							pL = Ratv(Term(iC + 1, CInt(i)), tW)
							If i <= 2 Or i = 9 Or i = 13 Or i > 14 Then
								pV1 = Ratv(CvTerm(iC + 1, CInt(i)), CvTerm(iC + 1, 9))
							Else
								pV1 = 0
							End If
							line_no += 1
							With gLSht.Range("A4")
								.Offset(line_no, 0).Value = TermName(CInt(i))
								.Offset(line_no, 4).Value = Term(iC + 1, CInt(i))
								.Offset(line_no, 4).NumberFormat = "0.0"
								If pL > 0 Then .Offset(line_no, 5).Value = pL
								.Offset(line_no, 5).NumberFormat = "0.0%"
								.Offset(line_no, 6).Value = CvTerm(iC + 1, CInt(i))
								.Offset(line_no, 6).NumberFormat = "0.00E+00"
								If pV1 > 0 Then .Offset(line_no, 7).Value = pV1
								.Offset(line_no, 7).NumberFormat = "0.0%"
								.Offset(line_no, 8).Value = cV
								.Offset(line_no, 8).NumberFormat = "0.00"
								If cU > 0 Then .Offset(line_no, 9).Value = cU
								.Offset(line_no, 9).NumberFormat = "0.0"
								If Export > 0 Then .Offset(line_no, 10).Value = Export
                                .Offset(line_no, 10).NumberFormat = "0.0"
                            End With

                        End If
					Next j

					'C MASS BALANCE STATISTICS
					For i = 1 To 8
						x(CInt(i)) = 0
					Next i

					'C OVERFLOW RATE M/YR  BASED ON NET INFLOW
					If Area(Nseg + 1) > 0 Then x(1) = MAx((Term(1, 9) - Term(1, 3)) / Area(Nseg + 1), 0)

					'C RESIDENCE TIME (YRS)
					If x(1) > 0 Then x(2) = Zmn(Nseg + 1) / x(1)

					'C MEAN POOL CONC
					'        x(3) = Cobs(Nseg + 1, iC)
					'If iO > 1 Then x(3) = Cest(Nseg + 1, ic)  ' ?????
					'        If Iop(11) = 1 Or x(3) <= 0 Then x(3) = Cest(Nseg + 1, iC) ' fixed 4/1/2004
					x(3) = Cuse(Cobs(Nseg + 1, iC), Cest(Nseg + 1, iC), Iop(11))

					'C NUTRIENT RESIDENCE TIME
					x(4) = Ratv(Area(Nseg + 1) * Zmn(Nseg + 1) * x(3), Term(iC + 1, 9))

					'C TURNOVER RATIO
					x(5) = Ratv(P(1), x(4))

					'C RETENTION COEF
					x(6) = Ratv(Term(iC + 1, 5), Term(iC + 1, 9))


					line_no += 2
					With gLSht.Range("A4")
						.Offset(line_no, 1).Value = "Overflow Rate (m/yr)"
						.Offset(line_no, 4).NumberFormat = "0.0"
						.Offset(line_no, 4).Value = x(1)

						line_no += 1
						.Offset(line_no, 1).Value = "Hydraulic Resid. Time (yrs)"
						.Offset(line_no, 4).Value = x(2)
						.Offset(line_no, 4).NumberFormat = "0.0000"

						line_no += 1
						.Offset(line_no, 1).Value = "Reservoir Conc (mg/m3)"
						.Offset(line_no, 4).Value = x(3)
						.Offset(line_no, 4).NumberFormat = "0"

						line_no -= 2
						.Offset(line_no, 6).Value = "Nutrient Resid. Time (yrs)"
						.Offset(line_no, 9).Value = x(4)
						.Offset(line_no, 9).NumberFormat = "0.0000"

						line_no += 1
						.Offset(line_no, 6).Value = "Turnover Ratio"
						.Offset(line_no, 9).Value = x(5)
						.Offset(line_no, 9).NumberFormat = "0.0"

						line_no += 1
						.Offset(line_no, 6).Value = "Retention Coef."
						.Offset(line_no, 9).Value = x(6)
						.Offset(line_no, 9).NumberFormat = "0.000"
					End With

				End If
			Next iC
		End With

	End Sub

	Sub Dumpa(ByVal io As Integer)
		Dim ioff As Integer

		'save mass balance solution matrix - debugging

		With Wkb.Sheets("matrix").Range("A1")
			If io = 0 Then
				ioff = 0
				.Cells.Clear()
				'.Range("A1:z999").ClearContents
			Else
				ioff = io * 12
			End If

			For i As Integer = 1 To Nseg
				For j As Integer = 1 To Nseg + 1
					If io < 2 Then
						.Offset(i + ioff, j - 1) = A(i, j)
					Else
						.Offset(i + ioff, j - 1) = Q(i, j)
					End If
				Next j
			Next i

		End With
	End Sub

	Sub List_Terms()
		Dim j As Integer

		'C IPRINT=5 SEGMENT WATER AND MASS BALANCES - FORMAT 2

		Dim k, iC As Integer

		'terms

		If NTrib <= 0 Then Exit Sub

		StartSheet("Summary Balances")

		With gLSht.Range("A3")
			line_no = 0
			For jc As Integer = 1 To 4
				iC = jc - 1

				If Not (iC > 0 And Iop(iC) <= 0) Then

					For i As Integer = 1 To 10
						y(i) = 0
					Next i

					If iC = 0 Then
						Lord(8) = 3 'water balance - uses evaporation in last slot
						line_no += 1
						Hdr.Range("Header_Wbal").Copy()
						gLSht.Paste(gLSht.Range("A3").Offset(line_no, 0))
						gLSht.Range("A3").Offset(line_no, 5).Value = P(1) 'Averaging Period
					Else
						Lord(8) = 5 'mass balance - uses net retention in last slot
						line_no += 2
						Hdr.Range("Header_Mbal").Copy()
						gLSht.Paste(gLSht.Range("A3").Offset(line_no, 0))
						gLSht.Range("A3").Offset(line_no, 8).Value = VariableName(Imap(iC))
						gLSht.Range("A3").Offset(line_no, 3).Value = MassBalName(Iop(11))
					End If
					line_no += 2

					For jseG As Double = 1 To Nseg
						k = MAx(iC, 1)
						Balas(jseG, k, Iop(11))
						For i As Integer = 1 To Mord
							j = Lord(i)
							If iC = 0 Then
								x(i) = Qt(j)
							Else
								x(i) = Bt(j)
							End If
						Next i


						If iC > 0 Then
							x(8) = Bt(5) - Bt(15) 'retention - internal load
							'x(7) = Bt(12) - Bt(8)   'net diffusive output
							x(7) = Bt(14) - Bt(13)
						Else
							x(7) = Exch(CInt(jseG))
						End If

						If Iout(CInt(jseG)) = 0 Then y(5) += x(5)
						For i As Integer = 1 To Mord
							If (i <> 5) Then y(i) += x(i)
						Next i
						line_no += 1
						With gLSht.Range("A3")
							.Offset(line_no, 0).Value = jseG
							.Offset(line_no, 0).HorizontalAlignment = Excel.Constants.xlCenter
							.Offset(line_no, 1).Value = SegName(CInt(jseG))
							For i As Integer = 1 To Mord
								.Offset(line_no, i + 1).Value = x(i)
								.Offset(line_no, i + 1).NumberFormat = "0"
							Next i
						End With

					Next jseG

					y(3) = 0
					y(7) = 0
					line_no += 1
					With gLSht.Range("A3")
						.Offset(line_no, 0).Value = "Net"
						For i As Integer = 1 To Mord
							.Offset(line_no, i + 1).Value = y(i)
							.Offset(line_no, i + 1).NumberFormat = "0"
						Next i
					End With

				End If
			Next jc

		End With
	End Sub

	Sub List_SegBalances()
		Dim qU, Tf, tW, xL As Double

		'C IPRINT=4  WATER AND NUTRIENT BALANCES BY SEGMENT
		Dim i As Integer
		Dim cU As Single

		If NTrib <= 0 Then Exit Sub
		StartSheet("Segment Balances")

		With gLSht.Range("A4")

			line_no = 0
			With gLSht.Range("A4").Offset(line_no, 0)
				.Value = "Segment Mass Balance Based Upon " & MassBalName(Iop(11)) & " Concentrations"
				.Font.Bold = True
			End With
			For jseG As Double = 1 To Nseg
				If Izap(CInt(jseG)) = 0 Then

					For iC As Integer = 1 To 3
						If Iop(iC) > 0 Then
							Balas(jseG, iC, Iop(11))

							line_no += 2
							Hdr.Range("Header_segbal").Copy()
							gLSht.Paste(gLSht.Range("A4").Offset(line_no, 0))

							With gLSht.Range("A4")
								.Offset(line_no, 2).Value = VariableName(Imap(iC))
								.Offset(line_no, 5).Value = jseG
								.Offset(line_no, 5).HorizontalAlignment = Excel.Constants.xlCenter
								.Offset(line_no, 6).Value = SegName(CInt(jseG))
							End With

							line_no += 2

							' 393    WRITE(NOUT,301) VariableName(IC),JSEG,SegName(JSEG)
							' 301    FORMAT(' COMPONENT: ',A8,20x,
							'     * '  SEGMENT:',I3,1X,a16/24x,
							'     * '     --- FLOW ---       --- LOAD ---         CONC'/
							'     * ' ID  T LOCATION         ',
							'     * '     HM3/YR      %       KG/YR      %       MG/M3')

							'C EXTERNAL LOAD COMPONENTS

							Tf = Qt(9)
							tW = Bt(9)
							For i = 1 To NTrib
								If Not (Iseg(i) <> jseG Or Icode(i) >= 5) Then
									If Icode(i) = 2 Then
										cU = Concil(i, Imap(iC))
										qU = FlowL(i)
									Else
										cU = Conci(i, Imap(iC))
										qU = Flow(i)
									End If
									'           If Icode(i) > 3 And Iop(11) = 1 Then cU = Cest(jseG, iC)
									If Icode(i) > 3 Then cU = Cuse(Cobs(CInt(jseG), iC), Cest(CInt(jseG), iC), Iop(11)) '4/11/2004

									xL = qU * cU
									If xL > 0 Then
										line_no += 1
										With gLSht.Range("A4")
											.Offset(line_no, 0).Value = i
											.Offset(line_no, 0).HorizontalAlignment = Excel.Constants.xlCenter
											.Offset(line_no, 1).Value = Icode(i)
											.Offset(line_no, 1).HorizontalAlignment = Excel.Constants.xlCenter
											.Offset(line_no, 2).Value = TribName(i)
											.Offset(line_no, 3).Value = qU
											.Offset(line_no, 3).NumberFormat = "0.0"
											.Offset(line_no, 4).Value = Ratv(qU, Tf)
											.Offset(line_no, 4).NumberFormat = "0.0%"
											.Offset(line_no, 5).Value = xL
											.Offset(line_no, 5).NumberFormat = "0.0"
											.Offset(line_no, 6).Value = Ratv(xL, tW)
											.Offset(line_no, 6).NumberFormat = "0.0%"
											If cU > 0 Then
												.Offset(line_no, 7).Value = cU
												.Offset(line_no, 7).NumberFormat = "0"
											End If
										End With
									End If
								End If
							Next i

							'C SUMMARY TERMS

							For j As Integer = 1 To Njord
								i = Jord(j)
								If Not (j < Njord And Bt(i) = 0# And Qt(i) = 0#) Then
									cU = 0
									If (i <> 5 And i <> 15) Then cU = Ratv(Bt(i), Qt(i))
									line_no += 1
									With gLSht.Range("A4")
										.Offset(line_no, 0).Value = TermName(i)
										.Offset(line_no, 3).Value = Qt(i)
										.Offset(line_no, 3).NumberFormat = "0.0"
										.Offset(line_no, 4).Value = Ratv(Qt(i), Tf)
										.Offset(line_no, 4).NumberFormat = "0.0%"
										.Offset(line_no, 5).Value = Bt(i)
										.Offset(line_no, 5).NumberFormat = "0.0"
										.Offset(line_no, 6).Value = Ratv(Bt(i), tW)
										.Offset(line_no, 6).NumberFormat = "0.0%"
										If cU > 0 Then
											.Offset(line_no, 7).Value = cU
											.Offset(line_no, 7).NumberFormat = "0"
										End If
									End With
								End If
							Next j

							Hydrau(jseG)

							'        WRITE(NOUT,333) x'(3),x(4),zmn(jseg)
							'333    FORMAT(
							'   * ' RESID. TIME =',F8.3,' YRS, OVERFLOW RATE =',F8.1,' M/YR',
							'  * ', DEPTH =',F6.1,' M')

							line_no += 2
							With gLSht.Range("A4")
								.Offset(line_no, 0).Value = "Hyd. Residence Time ="
								.Offset(line_no, 3).Value = x(3)
								.Offset(line_no, 3).NumberFormat = "0.0000"
								.Offset(line_no, 4).Value = " yrs"

								line_no += 1
								.Offset(line_no, 0).Value = "Overflow Rate ="
								.Offset(line_no, 3).Value = x(4)
								.Offset(line_no, 3).NumberFormat = "0.0"
								.Offset(line_no, 4).Value = " m/yr"

								line_no += 1
								.Offset(line_no, 0).Value = "Mean Depth ="
								.Offset(line_no, 3).Value = Zmn(CInt(jseG))
								.Offset(line_no, 3).NumberFormat = "0.0"
								.Offset(line_no, 4).Value = " m"
							End With

						End If
					Next iC
				End If
			Next jseG
		End With
	End Sub

	Sub List_TTests()
		Dim cV, i As Double
		Dim j, nord1 As Integer

		'C IPRINT=6 PRINT OBSERVED AND PREDICTED WQ

		line_no = 0
		StartSheet("T tests")

		With gLSht.Range("A4")

			Hdr.Range("Header_ttest1").Copy()
			gLSht.Paste(gLSht.Range("A4").Offset(line_no, 0))
			line_no += 3

			For ii As Double = 1 To Nseg + 1
				If ii = 1 Then
					i = Nseg + 1
				Else
					i = ii - 1
				End If

				If Izap(CInt(i)) = 0 And Not (Nseg = 1 And ii = 1) Then

					line_no += 2
					Hdr.Range("Header_ttest2").Copy()
					gLSht.Paste(gLSht.Range("A4").Offset(line_no, 0))
					If i <= Nseg Then gLSht.Range("A4").Offset(line_no, 1).Value = i
					gLSht.Range("A4").Offset(line_no, 1).HorizontalAlignment = Excel.Constants.xlCenter
					gLSht.Range("A4").Offset(line_no, 2).Value = SegName(CInt(i))
					line_no += 2

					nord1 = 13
					For l As Integer = 1 To nord1
						j = Iord(l)
						If Cobs(CInt(i), j) > 0 And Cest(CInt(i), j) > 0 Then 'list only paired results
							For k As Integer = 1 To 4
								x(k) = 0
							Next k
							cV = Ratv(Math.Sqrt(CvCest(CInt(i), j)), Cest(CInt(i), j))
							If Cest(CInt(i), j) > 0 And Cobs(CInt(i), j) > 0 Then
								x(1) = Cobs(CInt(i), j) / Cest(CInt(i), j)
								y(1) = CvCobs(CInt(i), j)
								y(2) = Stat(j, 3)
								y(3) = Math.Sqrt(cV ^ 2 + CvCobs(CInt(i), j) ^ 2)
								y(4) = Math.Log(x(1))
								For k As Integer = 1 To 3
									If y(k) > 0 Then x(k + 1) = y(4) / y(k)
								Next k
								If Iop(9) <= 0 Then x(4) = 0
							End If
							line_no += 1
							With gLSht.Range("A4")
								.Offset(line_no, 0).Value = DiagName(j)
								If Cobs(CInt(i), j) > 0 Then
									.Offset(line_no, 1).Value = Cobs(CInt(i), j)
									.Offset(line_no, 1).NumberFormat = "0.0"
									.Offset(line_no, 2).Value = CvCobs(CInt(i), j)
									.Offset(line_no, 2).NumberFormat = "0.00"
								End If
								.Offset(line_no, 3).Value = Cest(CInt(i), j)
								.Offset(line_no, 3).NumberFormat = "0.0"
								.Offset(line_no, 4).Value = cV
								.Offset(line_no, 4).NumberFormat = "0.00"
								For k As Integer = 1 To 4
									If x(1) > 0 And x(k) <> 0 Then
										.Offset(line_no, 4 + k).Value = x(k)
										.Offset(line_no, 4 + k).NumberFormat = "0.00"
									End If
								Next k
							End With
						End If
					Next l

				End If
			Next ii
		End With
	End Sub

	Sub List_Diagnostics(ByVal io As Integer)
		Dim i As Double
		Dim j, k As Integer
		Dim kseG, p1, p2, t As Double

		'c iprint=7 diagnostics  io=0 predicted, io=1 predicted & observed

		line_no = 0
		StartSheet("Diagnostics")

		If io = 0 Then
			Status("Predicted")
		Else
			Status("Predicted & Observed")
		End If

		With gLSht.Range("A4")
			If io = 1 Then
				gLSht.Range("A4").Offset(0, 0).Value = "Predicted & Observed Values Ranked Against CE Model Development Dataset"
			Else
				gLSht.Range("A4").Offset(0, 0).Value = "Predicted Values Ranked Against CE Model Development Dataset"
			End If
			gLSht.Range("A4").Offset(0, 0).Font.Bold = True

			If Nseg = 1 Then
				kseG = 1
			Else
				kseG = Nseg + 1
			End If

			For ii As Double = 1 To kseG
				If ii = 1 Then
					i = kseG
				Else
					i = ii - 1
				End If

				If Izap(CInt(i)) = 0 Then

					line_no += 2
					If io = 0 Then
						Hdr.Range("header_diag_0").Copy()
					Else
						Hdr.Range("Header_diag").Copy()
					End If
					gLSht.Paste(gLSht.Range("A4").Offset(line_no, 0))

					gLSht.Range("A4").Offset(line_no, 1).Value = i
					gLSht.Range("A4").Offset(line_no, 1).HorizontalAlignment = Excel.Constants.xlCenter
					gLSht.Range("A4").Offset(line_no, 2).Value = SegName(CInt(i))
					line_no += 2

					For l As Integer = 1 To Nord
						j = Iord(l)
						If Not (Cobs(CInt(i), j) <= 0 And Cest(CInt(i), j) <= 0) Then

							'ranking
							If j > 19 And j < 26 Then 'chla freq
								k = 4
							ElseIf j = 26 Then  'tsip
								k = 2
							ElseIf j = 27 Then  'tsi chla
								k = 4
							ElseIf j = 28 Then  'tsi secchi
								k = 5
							Else
								k = j
							End If
							p1 = 0
							If Cobs(CInt(i), k) > 0 Then Rank(Cobs(CInt(i), k), Stat(k, 1), Stat(k, 2), t, p1)
							p2 = 0
							If Cest(CInt(i), k) > 0 Then Rank(Cest(CInt(i), k), Stat(k, 1), Stat(k, 2), t, p2)
							If j = 28 Then 'reverse ranking for
								If p1 > 0 Then p1 = 1 - p1
								If p2 > 0 Then p2 = 1 - p2
							End If

							line_no += 1
							gLSht.Range("A4").Offset(line_no, 0).Value = DiagName(j)

							If Cest(CInt(i), j) > 0 Then
								gLSht.Range("A4").Offset(line_no, 1).Value = Cest(CInt(i), j)
								If CvCest(CInt(i), j) > 0 Then
									gLSht.Range("A4").Offset(line_no, 2).Value = Math.Sqrt(CvCest(CInt(i), j)) / Cest(CInt(i), j)
								End If
								If p2 > 0 Then
									gLSht.Range("A4").Offset(line_no, 3).Value = p2
								End If
								gLSht.Range("A4").Offset(line_no, 3).NumberFormat = "0.0%"
								gLSht.Range("A4").Offset(line_no, 1).NumberFormat = "0.0"
								gLSht.Range("A4").Offset(line_no, 2).NumberFormat = "0.00"
							End If

							If Cobs(CInt(i), j) > 0 And io > 0 Then
								gLSht.Range("A4").Offset(line_no, 4).Value = Cobs(CInt(i), j)
								If CvCobs(CInt(i), j) > 0 Then
									gLSht.Range("A4").Offset(line_no, 5).Value = CvCobs(CInt(i), j)
								End If
								If p1 > 0 Then
									gLSht.Range("A4").Offset(line_no, 6).Value = p1
								End If
								gLSht.Range("A4").Offset(line_no, 6).NumberFormat = "0.0%"
								gLSht.Range("A4").Offset(line_no, 4).NumberFormat = "0.0"
								gLSht.Range("A4").Offset(line_no, 5).NumberFormat = "0.00"
							End If



						End If

					Next l
				End If

			Next ii

		End With
	End Sub

	Sub Rank(ByVal x1 As Double, ByVal z1 As Double, ByVal z2 As Double, ByRef t As Double, ByRef perc As Double)
		perc = 0.5
		t = 0
		If x1 = z1 Then Exit Sub
		perc = 0
		If z2 * z1 <= 0 Or x1 <= 0 Then Exit Sub
		t = Math.Log(x1 / z1) / z2
		perc = Wka.Application.WorksheetFunction.NormSDist(t)
	End Sub

	Sub List_Profiles()
		Dim i, nord1 As Double

		'C IPRINT=8 PRINT PROFILES
		Dim kseG As Integer
		Dim sN As String = ""
		Dim kU As Integer

		line_no = 0
		StartSheet("Profiles")
		With gLSht.Range("A4")

			If Nseg = 1 Then
				kseG = 1
			Else
				kseG = Nseg + 1
			End If

			gLSht.Range("A4").Offset(line_no, 0).Value = "Segment"
			gLSht.Range("A4").Offset(line_no, 1).Value = " Name"
			gLSht.Range("A4").Offset(line_no, 1).Font.Bold = True
			gLSht.Range("A4").Offset(line_no, 0).Font.Bold = True
			gLSht.Range("A4").Offset(line_no, 0).HorizontalAlignment = Excel.Constants.xlCenter

			For j As Double = 1 To kseG
				line_no += 1
				If j = Nseg + 1 Then
					gLSht.Range("A4").Offset(line_no, 0).Value = "Mean"
					gLSht.Range("A4").Offset(line_no, 0).HorizontalAlignment = Excel.Constants.xlRight
				Else
					gLSht.Range("A4").Offset(line_no, 0).Value = j
				End If
				gLSht.Range("A4").Offset(line_no, 1).Value = " " & SegName(CInt(j))
			Next j

			line_no += 1

			For io As Integer = 1 To 5
				Select Case io
					Case 1
						sN = "PREDICTED CONCENTRATIONS:"
					Case 2
						sN = "OBSERVED CONCENTRATIONS:"
					Case 3
						sN = "OBSERVED/PREDICTED RATIOS:"
					Case 4
						sN = "OBSERVED STANDARD ERRORS"
					Case 5
						sN = "PREDICTED STANDARD ERRORS"
				End Select

				line_no += 2
				gLSht.Range("A4").Offset(line_no, 0).Value = sN
				gLSht.Range("A4").Offset(line_no, 0).Font.Bold = True

				line_no += 1

				'With .Range(.Offset(0, 0), .Offset(0, kseG + 1)).Font
				'    .Bold = True
				'    .Underline = True
				'End With

				For i = 1 To kseG
					gLSht.Range("A4").Offset(line_no, 0).Value = "Variable  Segment-->"
					If i = Nseg + 1 Then
						gLSht.Range("A4").Offset(line_no, i + 1).Value = "Mean"
						gLSht.Range("A4").Offset(line_no, i + 1).HorizontalAlignment = Excel.Constants.xlRight
					Else
						gLSht.Range("A4").Offset(line_no, i + 1).Value = i
					End If
					gLSht.Range("A4").Offset(line_no, 0).Font.Bold = True
					gLSht.Range("A4").Offset(line_no, i + 1).Font.Bold = True
					gLSht.Range("A4").Offset(line_no, 0).Font.Underline = True
					gLSht.Range("A4").Offset(line_no, i + 1).Font.Underline = True
				Next i

				'       nord1 = 10
				nord1 = NDiagnostics
				For j As Double = 1 To nord1
					i = Iord(CInt(j))
					kU = 0
					For k As Double = 1 To kseG
						x(CInt(k)) = 0

						Select Case io
							Case 1
								x(CInt(k)) = Cest(CInt(k), CInt(i))
							Case 2
								x(CInt(k)) = Cobs(CInt(k), CInt(i))
							Case 3
								x(CInt(k)) = Ratv(Cobs(CInt(k), CInt(i)), Cest(CInt(k), CInt(i)))
							Case 4
								x(CInt(k)) = CvCobs(CInt(k), CInt(i)) * Cobs(CInt(k), CInt(i))
							Case 5
								x(CInt(k)) = Math.Sqrt(CvCest(CInt(k), CInt(i)))
						End Select
						If x(CInt(k)) > 0 Then kU = 1
					Next k

					If kU > 0 Then
						line_no += 1
						With gLSht.Range("A4").Offset(line_no, 0)
							.Value = DiagName(i)
						End With

						For k As Integer = 1 To kseG
							Dim targetCell As Excel.Range = gLSht.Range("A4").Offset(line_no, k + 1)
							If x(k) > 0 Then targetCell.Value = x(k)
							targetCell.NumberFormat = "0.0"
						Next
					End If

				Next j

			Next io

		End With

	End Sub

	Sub List_Verify()
		Dim be, j, wbe As Double
		'c verify segment balances

		If Iop(1) > 0 Then
			j = 1
		Else
			j = 2
		End If

		StartSheet("verify")
		line_no = 0
		With gLSht.Range("A4")
			Hdr.Range("header_verify").Copy()
			gLSht.Paste(gLSht.Range("A4").Offset(0, 0))
			' Set the variable name in the appropriate cell
			gLSht.Range("A4").Offset(0, 4).Value = VariableName(CInt(j))
			line_no += 2

			For i As Double = 1 To Nseg
				'Call Balas(i, j, 2)
				Balas(i, j, 1)
				wbe = Ratv(Qt(5), Qt(9))
				be = Ratv(Bt(5), Bt(9))
				line_no += 1
				gLSht.Range("A4").Offset(line_no, 0).Value = i
				gLSht.Range("A4").Offset(line_no, 0).HorizontalAlignment = Excel.Constants.xlCenter
				gLSht.Range("A4").Offset(line_no, 1).Value = SegName(CInt(i))
				gLSht.Range("A4").Offset(line_no, 2).Value = Qt(9)
				gLSht.Range("A4").Offset(line_no, 2).NumberFormat = "0"
				gLSht.Range("A4").Offset(line_no, 3).Value = Qt(5)
				gLSht.Range("A4").Offset(line_no, 3).NumberFormat = "0"
				gLSht.Range("A4").Offset(line_no, 4).Value = wbe
				gLSht.Range("A4").Offset(line_no, 4).NumberFormat = "0.0%"
				gLSht.Range("A4").Offset(line_no, 5).Value = Bt(9)
				gLSht.Range("A4").Offset(line_no, 5).NumberFormat = "0"
				gLSht.Range("A4").Offset(line_no, 6).Value = Bt(5)
				gLSht.Range("A4").Offset(line_no, 6).NumberFormat = "0"
				gLSht.Range("A4").Offset(line_no, 7).Value = be
				gLSht.Range("A4").Offset(line_no, 7).NumberFormat = "0.0%"
				gLSht.Range("A4").Offset(line_no, 8).Value = Qadv(CInt(i))
				gLSht.Range("A4").Offset(line_no, 8).NumberFormat = "0"
			Next i

		End With

		'          write(nout,12) i,SegName(i),qt(9),qt(5),wbe,bt(9),bt(5),be,
		'     &      qadv(i)
		' 12     format(i3,1x,a8,f11.2,f10.2,f6.2,f11.2,f10.2,f6.2,f10.2)

	End Sub
	Function StrSp(ByVal j As Integer, ByVal s As String) As String
		'returns string of fixed length j
		Return (s & New String(" "c, j)).Substring(0, Math.Min(j, (s & New String(" "c, j)).Length))
	End Function

	Sub List_Tree()
		'List Segment & Tributary Network

		SegName(0) = "Out of Reservoir"
		StartSheet("Segment Network")
		line_no = 0
		With gLSht.Range("A4")

			gLSht.Range("A4").Offset(line_no, 0).Value = "Segment & Tributary Network"
			gLSht.Range("A4").Offset(line_no, 0).Font.Bold = True

			For i As Double = 1 To Nseg

				line_no += 2
				gLSht.Range("A4").Offset(line_no, 0).Value = "--------Segment:"
				gLSht.Range("A4").Offset(line_no, 0).HorizontalAlignment = Excel.Constants.xlRight
				gLSht.Range("A4").Offset(line_no, 1).Value = i
				gLSht.Range("A4").Offset(line_no, 1).HorizontalAlignment = Excel.Constants.xlCenter
				gLSht.Range("A4").Offset(line_no, 2).Value = SegName(CInt(i))

				line_no += 1
				gLSht.Range("A4").Offset(line_no, 0).Value = "Outflow Segment:"
				gLSht.Range("A4").Offset(line_no, 0).HorizontalAlignment = Excel.Constants.xlRight
				gLSht.Range("A4").Offset(line_no, 1).Value = Iout(CInt(i))
				gLSht.Range("A4").Offset(line_no, 1).HorizontalAlignment = Excel.Constants.xlCenter
				gLSht.Range("A4").Offset(line_no, 2).Value = SegName(Iout(CInt(i)))

				For j As Double = 1 To NTrib
					If Iseg(CInt(j)) = i Then
						line_no += 1
						gLSht.Range("A4").Offset(line_no, 0).Value = "Tributary:"
						gLSht.Range("A4").Offset(line_no, 0).HorizontalAlignment = Excel.Constants.xlRight
						gLSht.Range("A4").Offset(line_no, 1).Value = j
						gLSht.Range("A4").Offset(line_no, 1).HorizontalAlignment = Excel.Constants.xlCenter
						gLSht.Range("A4").Offset(line_no, 2).Value = TribName(CInt(j))
						gLSht.Range("A4").Offset(line_no, 3).Value = "Type:"
						gLSht.Range("A4").Offset(line_no, 3).HorizontalAlignment = Excel.Constants.xlRight
						gLSht.Range("A4").Offset(line_no, 4).Value = Type_Code(Icode(CInt(j)))
					End If
				Next j

				For j As Double = 1 To Npipe

					If Ifr(CInt(j)) = i Then
						line_no += 1
						gLSht.Range("A4").Offset(line_no, 0).Value = "Channel:"
						gLSht.Range("A4").Offset(line_no, 0).HorizontalAlignment = Excel.Constants.xlRight
						gLSht.Range("A4").Offset(line_no, 1).Value = j
						gLSht.Range("A4").Offset(line_no, 1).HorizontalAlignment = Excel.Constants.xlCenter
						gLSht.Range("A4").Offset(line_no, 2).Value = PipeName(CInt(j))
						gLSht.Range("A4").Offset(line_no, 3).Value = " To Segment:"
						gLSht.Range("A4").Offset(line_no, 3).HorizontalAlignment = Excel.Constants.xlRight
						gLSht.Range("A4").Offset(line_no, 4).Value = StringsHelper.Format(Ito(CInt(j)), "00") & " " & SegName(Ito(CInt(j)))
					End If
				Next j

			Next i
		End With
	End Sub

	Sub List_Tree2()
		Dim npipes As Double
		'List Segment & Tributary Network


		SegName(0) = "Out of Reservoir"
		Dim txt As New StringBuilder()
		txt.Append(Title & Environment.NewLine)
		txt.Append("Segment & Tributary Network" & Environment.NewLine & Environment.NewLine)

		For i As Double = 1 To Nseg
			txt.Append("----Segment: " & StringsHelper.Format(i, "00") & " " & SegName(CInt(i)) & Environment.NewLine)
			txt.Append("Outflow Seg: " & StringsHelper.Format(Iout(CInt(i)), "00") & " " & SegName(Iout(CInt(i))) & Environment.NewLine)
			For j As Double = 1 To NTrib
				If Iseg(CInt(j)) = i Then txt.Append("  Tributary: " & StringsHelper.Format(j, "00") & " " & StrSp(20, TribName(CInt(j))) & "   Type: " & Type_Code(Icode(CInt(j))) & Environment.NewLine)
			Next j
			For j As Double = 1 To npipes
				If Ifr(CInt(j)) = i Then txt.Append("    Channel: " & StringsHelper.Format(j, "00") & " " & StrSp(20, PipeName(CInt(j))) & "    To Segment: " & Ito(CInt(j)).ToString() & " " & SegName(Ito(CInt(j))) & Environment.NewLine)
			Next j
			txt.Append(Environment.NewLine)
		Next i

		With frmBox.DefInstance
			.txtBox.Text = txt.ToString()
			.txtBox.SelectionStart = 0
			.ShowDialog()
		End With

	End Sub
	Sub List_inss()
		Status("Inputs")
		gLSht = Wkb.Sheets("Inputs")
		Save_xls() 'save inputs to wkb
		If Ier > 0 Then Exit Sub
		'    Set gLSht = Wkb.Worksheets("inputs")
		ViewSheet("inputs")
		Status("Ready")

	End Sub
	Sub List_Inputs()
		Dim t As Double
		Dim k As Integer
		StartSheet("Case Data")

		line_no = 0
		With gLSht.Range("A3")

			' Copy the header_in_top range from Hdr to the target location in gLSht
			Hdr.Range("header_in_top").Copy()
			gLSht.Paste(gLSht.Range("A3").Offset(line_no, 0))

			For i As Integer = 1 To 10
				line_no += 1
				gLSht.Range("A3").Offset(line_no, 1).Value = Note(i)
				gLSht.Range("A3").Offset(line_no, 1).WrapText = False
			Next i

			'global variables
			line_no += 2
			k = line_no
			For i As Integer = 1 To 4
				line_no += 1
				gLSht.Range("A3").Offset(line_no, 2).Value = P(i)
				gLSht.Range("A3").Offset(line_no, 3).Value = Cp(i)
			Next i

			'atmospheric loads
			line_no += 2
			For i As Integer = 1 To NVariables
				line_no += 1
				gLSht.Range("A3").Offset(line_no, 2).Value = Atm(i)
				gLSht.Range("A3").Offset(line_no, 3).Value = CvAtm(i)
			Next i

			'Model Options
			line_no = k
			For i As Integer = 1 To NOptions
				line_no += 1
				gLSht.Range("A3").Offset(line_no, 9).Value = Iop(i)
				gLSht.Range("A3").Offset(line_no, 9).HorizontalAlignment = Excel.Constants.xlCenter
				gLSht.Range("A3").Offset(line_no, 10).Value = OptionName(i, Iop(i) + 1)
			Next i

			'segment list
			line_no += 4
			For i As Integer = 1 To Nseg
				line_no += 1
				gLSht.Range("A3").Offset(line_no, 0).Value = i
				gLSht.Range("A3").Offset(line_no, 0).HorizontalAlignment = Excel.Constants.xlCenter
				gLSht.Range("A3").Offset(line_no, 1).Value = SegName(i)
				gLSht.Range("A3").Offset(line_no, 3).Value = Iout(i)
				gLSht.Range("A3").Offset(line_no, 4).Value = Iag(i)
				gLSht.Range("A3").Offset(line_no, 5).Value = Area(i)
				gLSht.Range("A3").Offset(line_no, 6).Value = Zmn(i)
				gLSht.Range("A3").Offset(line_no, 7).Value = Slen(i)
				gLSht.Range("A3").Offset(line_no, 8).Value = Zmx(i)
				gLSht.Range("A3").Offset(line_no, 9).Value = CvZmx(i)
				gLSht.Range("A3").Offset(line_no, 10).Value = Zhyp(i)
				gLSht.Range("A3").Offset(line_no, 11).Value = CvZhyp(i)
				gLSht.Range("A3").Offset(line_no, 12).Value = Turbi(i)
				gLSht.Range("A3").Offset(line_no, 13).Value = CvTurbi(i)
				For j As Integer = 1 To 3
					gLSht.Range("A3").Offset(line_no, 12 + j * 2).Value = InternalLoad(i, j)
					gLSht.Range("A3").Offset(line_no, 12 + j * 2 + 1).Value = CvInternalLoad(i, j)
				Next j
			Next i
			line_no += 2

			' segment observed water quality
			Hdr.Range("header_in_obswq").Copy()
			gLSht.Paste(gLSht.Range("A3").Offset(line_no, 0))
			line_no += 2
			For i As Integer = 1 To Nseg
				line_no += 1
				gLSht.Range("A3").Offset(line_no, 0).Value = i
				gLSht.Range("A3").Offset(line_no, 0).HorizontalAlignment = Excel.Constants.xlCenter
				For j As Integer = 1 To 9
					gLSht.Range("A3").Offset(line_no, -1 + j * 2).Value = Cobs(i, j)
					gLSht.Range("A3").Offset(line_no, 0 + j * 2).Value = CvCobs(i, j)
				Next j
			Next i

			line_no += 2

			' calibration factors
			Hdr.Range("header_in_calibfactors").Copy()
			gLSht.Paste(gLSht.Range("A3").Offset(line_no, 0))
			line_no += 2
			For i As Integer = 1 To Nseg
				line_no += 1
				gLSht.Range("A3").Offset(line_no, 0).Value = i
				gLSht.Range("A3").Offset(line_no, 0).HorizontalAlignment = Excel.Constants.xlCenter
				For j As Integer = 1 To 9
					gLSht.Range("A3").Offset(line_no, -1 + j * 2).Value = Cal(i, j)
					gLSht.Range("A3").Offset(line_no, 0 + j * 2).Value = CvCal(i, j)
				Next j
			Next i

			' tributaries
			line_no += 2
			Hdr.Range("header_in_tribs").Copy()
			gLSht.Paste(gLSht.Range("A3").Offset(line_no, 0))
			line_no += 2
			For i As Integer = 1 To NTrib
				line_no += 1
				gLSht.Range("A3").Offset(line_no, 0).Value = i
				gLSht.Range("A3").Offset(line_no, 0).HorizontalAlignment = Excel.Constants.xlCenter
				gLSht.Range("A3").Offset(line_no, 1).Value = TribName(i)
				gLSht.Range("A3").Offset(line_no, 3).Value = Iseg(i)
				gLSht.Range("A3").Offset(line_no, 4).Value = Icode(i)
				gLSht.Range("A3").Offset(line_no, 5).Value = Darea(i)
				gLSht.Range("A3").Offset(line_no, 6).Value = Flow(i)
				gLSht.Range("A3").Offset(line_no, 7).Value = CvFlow(i)
				For j As Integer = 1 To NVariables
					gLSht.Range("A3").Offset(line_no, 6 + 2 * j).Value = Conci(i, j)
					gLSht.Range("A3").Offset(line_no, 7 + 2 * j).Value = CvCi(i, j)
				Next j
			Next i

			' trib nonpoint areas
			t = 0
			For i As Integer = 1 To NTrib
				For j As Integer = 1 To NCAT
					t += Warea(i, j)
				Next j
			Next i

			If t > 0 Then
				line_no += 2
				Hdr.Range("header_in_areas").Copy()
				gLSht.Paste(gLSht.Range("A3").Offset(line_no, 0))
				line_no += 2
				For i As Integer = 1 To NTrib
					line_no += 1
					gLSht.Range("A3").Offset(line_no, 0).Value = i
					gLSht.Range("A3").Offset(line_no, 0).HorizontalAlignment = Excel.Constants.xlCenter
					gLSht.Range("A3").Offset(line_no, 1).Value = TribName(i)
					For j As Integer = 1 To NCAT
						gLSht.Range("A3").Offset(line_no, 2 + j).Value = Warea(i, j)
					Next j
				Next i
			End If

			' Channels

			If Npipe > 0 Then
				line_no += 2
				Hdr.Range("header_in_transport").Copy()
				gLSht.Paste(gLSht.Range("A3").Offset(line_no, 0))
				line_no += 2
				For i As Integer = 1 To Npipe
					line_no += 1
					gLSht.Range("A3").Offset(line_no, 0).Value = i
					gLSht.Range("A3").Offset(line_no, 0).HorizontalAlignment = Excel.Constants.xlCenter
					gLSht.Range("A3").Offset(line_no, 1).Value = PipeName(i)
					gLSht.Range("A3").Offset(line_no, 3).Value = Ifr(i)
					gLSht.Range("A3").Offset(line_no, 4).Value = Ito(i)
					gLSht.Range("A3").Offset(line_no, 5).Value = Qpipe(i)
					gLSht.Range("A3").Offset(line_no, 6).Value = CvQpipe(i)
					gLSht.Range("A3").Offset(line_no, 7).Value = Epipe(i)
					gLSht.Range("A3").Offset(line_no, 8).Value = CvEpipe(i)
				Next i
			End If

			'export categories
			t = 0
			For i As Integer = 1 To NCAT
				t += Ur(i)
			Next i

			If t > 0 Then
				line_no += 2
				Hdr.Range("header_in_export").Copy()
				gLSht.Paste(gLSht.Range("A3").Offset(line_no, 0))
				line_no += 2
				For i As Integer = 1 To NCAT
					line_no += 1
					gLSht.Range("A3").Offset(line_no, 0).Value = i
					gLSht.Range("A3").Offset(line_no, 14).HorizontalAlignment = Excel.Constants.xlCenter
					gLSht.Range("A3").Offset(line_no, 1).Value = LandUseName(i)
					gLSht.Range("A3").Offset(line_no, 3).Value = Ur(i)
					gLSht.Range("A3").Offset(line_no, 4).Value = CvUr(i)
					For k = 1 To NVariables
						gLSht.Range("A3").Offset(line_no, 3 + k * 2).Value = Uc(i, k)
						gLSht.Range("A3").Offset(line_no, 4 + k * 2).Value = CvUc(i, k)
					Next k
				Next i
			End If

			'coefficients
			line_no += 2
			Hdr.Range("header_in_last").Copy()
			gLSht.Paste(gLSht.Range("A3").Offset(line_no, 0))
			For i As Integer = 1 To NXk
				line_no += 1
				gLSht.Range("A3").Offset(line_no, 3).Value = Xk(i)
				gLSht.Range("A3").Offset(line_no, 4).Value = CvXk(i)
			Next i

		End With
	End Sub
End Module